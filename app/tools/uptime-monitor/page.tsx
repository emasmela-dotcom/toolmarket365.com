...
export default function UptimeMonitorPage() {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-4">
      <h1 className="text-2xl font-bold text-mono-950 dark:text-mono-50">Uptime monitor</h1>
      <p className="text-sm text-mono-600 dark:text-mono-400">
        Plan checks, alerts, and payloads. This app does not ping your servers — use a dedicated
        uptime provider for real probes.
      </p>
      <textarea
      ...
      >
        {loading ? "Building plan…" : "Build monitoring plan"}
      </button>
      {result ? (
        <div className="space-y-4 text-sm border border-mono-200 dark:border-mono-700 rounded p-4">
          {result.validUrls.length ? (
            <p className="text-mono-800 dark:text-mono-200">
              <span className="font-medium">Valid URLs:</span> {result.validUrls.join(", ")}
            </p>
          ) : null}
          {result.invalidEntries.length ? (
            <p className="text-amber-800 dark:text-amber-200">
              Skipped: {result.invalidEntries.join(" | ")}
            </p>
          ) : null}
          <div>
            <p className="font-semibold text-mono-900 dark:text-mono-100 mb-1">Checklist</p>
            <ul className="list-disc pl-5 text-mono-700 dark:text-mono-300 space-y-1">
              {result.monitoringChecklist.map((c, i) => (
                <li key={i}>{c}</li>
              ))}
            </ul>
          </div>
          <div>
            <p className="font-semibold text-mono-900 dark:text-mono-100 mb-1">Email alert</p>
            <p className="text-mono-600 dark:text-mono-400">{result.downtimeAlertEmail.subject}</p>
            <pre className="whitespace-pre-wrap text-xs bg-mono-100 dark:bg-mono-900 p-2 rounded mt-1">
              {result.downtimeAlertEmail.body}
            </pre>
          </div>
          <div>
            <p className="font-semibold text-mono-900 dark:text-mono-100 mb-1">Slack example JSON</p>
            <pre className="whitespace-pre-wrap text-xs bg-mono-100 dark:bg-mono-900 p-2 rounded overflow-x-auto">
